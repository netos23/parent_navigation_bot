...
import ru.fbtw.navigator.parent_navigation_bot.io.ImageUtils;
...
public class SearchingService {
	...
	public boolean hasName(String text) {
		return nodesStorage.containsKey(text.toLowerCase(Locale.ROOT));
	}

	public SendPhoto[] getPath(String from, String to) {
		try {
			List<GraphNode> path = solver.getPath(to.toLowerCase(Locale.ROOT),
	...
	public SendPhoto getLevel(String text) {
		try {
			Node node = nodesStorage.get(text);
			BufferedImage image = ImageUtils.imageFromBase64(node.getParent().getImage());

			InputStream is = getBufferedImageIs(image);
			InputFile file = new InputFile(is, "room.png");
			// File file = getFileFromBI(image);
